...
def show_dashboard():
    ...
    if not df_v_dash.empty:
        ...
        # Meta total (por ahora solo Noble tiene meta fija; para los demás, asumimos 0 si no existe)
        meta_noble = 145000.0
        ...
        c1.metric("Venta total del mes", f"${venta_acum_dash:,.2f}")
        c2.metric("Meta mensual (Noble)", f"${meta_noble:,.2f}")
        ...
        if "Canal" in df_v_mes_dash.columns:
            ...
            for i, canal in enumerate(CANALES_VENTA):
                ...
                cols_canales[i].metric(f"🏢 {canal}", f"${monto:,.2f}")
        else:
            st.info("Los datos históricos aún no tienen canal asignado. Se mostrarán correctamente después del primer registro con el nuevo sistema.")
    else:
