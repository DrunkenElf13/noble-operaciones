...
def show_dashboard_ventas():
    ...
    if canal_sel == "Noble" or (canal_sel == "Todos" and "Noble" in df_mes["Canal"].unique()):
        ...
        if not df_noble.empty:
            ...
            st.dataframe(df_disp.style.apply(
                lambda row: ["background-color: rgba(80,200,120,0.15)"
                             if limpiar_valor(row.get("Venta_Diaria", 0)) >= limpiar_valor(row.get("Meta_Diaria", 0))
                             else "background-color: rgba(239,159,39,0.15)"
                             if limpiar_valor(row.get("Venta_Diaria", 0)) >= limpiar_valor(row.get("Meta_Diaria", 0)) * 0.7
    ...
    if canal_sel in ["Coffee Station", "Todos"]:
        ...
        if not df_cs.empty:
            total_venta = df_cs["Venta_Total"].sum()
            total_cobrado = df_cs["Venta_Diaria"].sum()
            adeudo_total = df_cs["Adeudo"].sum() if "Adeudo" in df_cs.columns else 0.0
            anticipo_total = df_cs["Anticipo"].sum() if "Anticipo" in df_cs.columns else 0.0
            num_eventos = len(df_cs)
            c1, c2, c3, c4, c5 = st.columns(5)
            c1.metric("Venta Total", f"${total_venta:,.2f}")
            c2.metric("Venta Cobrada", f"${total_cobrado:,.2f}")
            c3.metric("Adeudo", f"${adeudo_total:,.2f}")
            c4.metric("Anticipo", f"${anticipo_total:,.2f}")
            c5.metric("Eventos", num_eventos)
        else:
    ...
    if canal_sel in ["Noble To Go", "Todos"]:
        ...
        if not df_ntg.empty:
            total_venta = df_ntg["Venta_Total"].sum()
            total_cobrado = df_ntg["Venta_Diaria"].sum()
            adeudo_total = df_ntg["Adeudo"].sum() if "Adeudo" in df_ntg.columns else 0.0
            anticipo_total = df_ntg["Anticipo"].sum() if "Anticipo" in df_ntg.columns else 0.0
    ...
    st.divider()
    with st.expander(f"📋 Detalle de canales adicionales ({calendar.month_name[mes_sel]} {año_sel})", expanded=False):
        ...
        for nombre_hoja, tab, prefijo in [
            ...
            with tab:
                ...
                st.dataframe(df_canal.drop(columns=["_fecha_dt"], errors="ignore"),
                             hide_index=True, use_container_width=True)
